...
hv_summary <- import(here("data", "highly_vulnernable_bilat_trade_summary.csv"))
hv_trade <- import(here("data", "highly_vulnernable_bilat_trade.csv"))
```
...
Therefore, I model the target and imposing states' utility over the good as functions of the tariff and the imposing state's type: $\mu_i(\tau, \zeta_I)$, where $i = \{I, T\}$.

The imposing state's utility, $\mu_I(\tau, \zeta_I)$ is assumed to have an interior maximum at $\bar{\tau}$. The target's utility over the good, $\mu_T(\tau, \zeta_I)$, is assumed to be concave in $\tau$ and monotonically decreasing in $\tau$. In other words, the target state always prefers a lower tariff. The imposing state has to work within the constraints of balancing the benefits of a higher tariff, including potential higher tax revenues, with providing its economy with access to an in-demand good at a cheaper price than could otherwise be accessed.
...
Can the target state locate alternative trade partners to fill the gap left by the imposing state? To measure this, I use annual bilateral trade data describing trade between the most dominant trading state and all of their trade partners for all commodities identified to be traded in highly concentrated markets. Attracted by the higher prices exports would receive or lower prices at which imports could be accessed, could alternative trade partners divert their trade in the vulnerable commodity to the target and fill the gap?[^3] For `r hv_summary |> distinct(direction, reporter, partner, cmd_code) |> nrow()` dyad-commodity pairings, the potential imposing state dominates trade to such an extent that all alternative trade partners combined cannot fill the gap they could leave. For these commodities, all other trading states combined would need to increase production by a factor of at least `r hv_summary |> summarise(min(gap_prop)) |> round(2)` and up to `r hv_summary |> summarise(max(gap_prop)) |> round(2)` to fill the gap left by the imposing state. A full list of these products and trade partners is provided in the appendix. For the remaining dyad-commodity pairings, trade diverted from some combination of alternative trade partners could fill the gap left by the imposing state.
...
hv_summary |> 
  ...
  select(!c(trade_value, alt_total_value, gap, gap_never_filled, gdp)) |>
